...
void YellowBall::CreateSpherewithNormal(void)
{
	int count;
	count = 0;
...
YellowBall::YellowBall(float s, float m, vec3 col, vec3 pos, string tag) : GameObject(1.0f, 0.5f, 100, pos, tag)
{
	speed = s;
	mass = m;
	position = pos;
...
VertexWtihNormal* YellowBall::GetVerData(int& verNum)
{
	verNum = 121;
	return sphereVerticesNor;
}

unsigned int* YellowBall::GetTriData(int& triNum)
{
	triNum = 660;
	return sphereIndices;
}

void YellowBall::SetPosition(vec3 newPos)
{
	position = newPos;
}

vec3 YellowBall::GetPosition(void)
{
	return position;
}

void YellowBall::Draw(int location, int colourLoc)
{
...
void YellowBall::Update(float deltaTime)
{
